...
class NovelPubNode(Node):
    def __init__(self, node_name):
        ...
        self.timer_period = 5.0  # 设置定时器周期为1秒
        self.timer_ = self.create_timer(self.timer_period, self.timer_callback) # 创建定时器
    ...
    def download_novel(self, url):
        ...
        for i in range(10):
            self.novels_queue_.put("123")
            self.novels_queue_.put("456")
            self.novels_queue_.put("789")
